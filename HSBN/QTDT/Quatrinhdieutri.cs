...
namespace HSBN.QTDT
{
    public partial class Quatrinhdieutri : Form
    {
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrWhiteSpace(ma) ||
               string.IsNullOrWhiteSpace(bn) ||
                ...
                string.IsNullOrWhiteSpace(thuoc))
            {
            ...
            try
            {
                foreach (xls.Worksheet ws in wb.Sheets)
                {
                    ...
                    while (ws.Cells[i, 1].Value != null)
                    {
                        ...
                        string bs = ws.Cells[i, 3].Value?.ToString().Trim();
                        string cd = ws.Cells[i, 5].Value?.ToString().Trim();
                        string pp = ws.Cells[i, 6].Value?.ToString().Trim();
                        string khoa = ws.Cells[i, 7].Value?.ToString().Trim();
                        string thuoc = ws.Cells[i, 8].Value?.ToString().Trim();
                        ...
                        qtdthaha(ma, bn, bs, ngaySinh, cd, pp, khoa, thuoc);
                        i++;
                    }
                }
        ...
        private void qtdthaha(string ma, string bn, string bs, DateTime ngaySinh, string cd, string pp, string khoa, string thuoc)
        {
            String ngaySinhSql = ngaySinh.ToString("yyyy-MM-dd");
            string sql = "Insert QuaTrinhDieuTri Values('" + ma + "',N'" + bn + "',N'" + bs + "','" + ngaySinh + "',N'" + cd + "',N'" + pp + "',N'" + khoa + "',N'" + thuoc + "')";
            thuvien.insert(sql);
